...
Consider a system with its state variable denoted by $\vfield{u}$, consisting of $p$ parameters given by the vector $\vecmu$, and an evolution law given by
\begin{equation}
...
The evolution law may arise from a finite set of ordinary differential equations or, may be a finite representation of an infinite dimensional partial differential equation, as often arises from the numerical discretization of the space-time evolution equations of physical systems. Typically, physical systems will have real parameters $\vecmu$ however, allowing for complex parameters presents no additional difficulty and we assume that $\vecmu \in \mathds{C}^{p}$. Without loss of generality one may assume $\fieldu = \fieldub$ to be a fixed point of the system for a certain set of parameters $\vecmu = \vecMu$, \ie,
\begin{align}
	\label{eqn:fixed_point}
	\fieldf(\fieldub,\vecMu) = \veczero.
\end{align}
We may consider the evolution of the deviation from the fixed point solution $\vecu = \vfield{u} - \vfield{u}^{b}$, and divide the evolution operator into its linear and nonlinear parts as,
\begin{align}
...
\end{align}
where, $\Lu\in\mathds{C}^{N\times N}$, and $\mathcal{N}(\vecu,\vecMu)\in\mathds{C}^{N}$. Furthermore, it is assumed that the fixed point is such that the system undergoes a multiple co-dimension bifurcation and the linearized operator $\Lu$ has a discrete set of $n$ critical eigenvalues $\lambda^{C}_{i},\ i\in 1,\ldots,n$, with zero growth rate $(\mathfrak{Re}(\lambda^{C}_{i}) = 0)$. This includes both purely real critical eigenvalues with $\mathfrak{Re}(\lambda^{C}_{i}) = 0,\ \mathfrak{Im}(\lambda^{C}_{i}) = 0$, and purely imaginary critical eigenvalues with $\mathfrak{Re}(\lambda^{C}_{i}) = 0,\ \mathfrak{Im}(\lambda^{C}_{i}) \ne 0$. Accordingly, the system has $n$ critical eigenpairs $\Tuple{\lambda^{C}_{i},\Devec_{i}}, i\in 1\ldots n$, where $\Devec_{i}$ represents the eigenvector corresponding to $\lambda^{C}_{i}$. For real systems the purely imaginary critical eigenvalues will occur in conjugate pairs. 

The linearized operator $\Lu$ will also be referred to as the direct (linear) operator. Its adjoint is denoted as $\Lu^{\dagger}$ which also has $n$ critical eigenpairs $\Tuple{\lambda^{C*}_{i}, \Aevec_{i}}$. The scaling of the eigenvectors of the direct and adjoint operators may be defined such that they satisfy the biorthogonality condition,
\begin{eqnarray}
	\langle\Aevec_{i},\Devec_{j}\rangle = \Aevec^{\dagger}_{i}\Devec_{j} = \delta_{i,j} \label{eqn:std_biorthogonality},
\end{eqnarray}
where $\langle\cdot,\cdot\rangle$ is the standard inner-product and $\delta_{i,j}$ is the Kronecker-delta function. The superscript $^{\dagger}$ is used here to indicate a complex-conjugated transpose of vectors and matrices. For scalars complex-conjugation is indicated by a $^{*}$ superscript. The superscript $^{H}$ which is often used for the same purpose is reserved for objects referring to harmonic forcings. One may define the matrices $\DevMatrix$ and $\AevMatrix$, whose columns comprise of the $n$ critical eigenvectors of $\Lu$ and $\Lu^{\dagger}$as,
\begin{subequations}
	...
	\end{align}
\end{subequations}
Note that in the manuscripts there will be various matrices and vectors that will be stacked either along columns or along rows as the requirement arises. We make a distinction through the use of comma $(,)$ or semicolon $(;)$ as the separator of elements. Hence elements stacked along a row are written as $[a,b,c,\ldots]$, as in equation~\eqref{eqn:eigenvectors_std} for example. Whereas, elements denoted as $[a;b;c;\ldots]$ are stacked along a column as will occur later in the manuscript. This allows the representation of long columns compactly.  
...
\end{align}
one may again separate out the linear and nonlinear parts as,
\begin{align}
...
\end{align}
where, $\Lu\in\mathds{C}^{N\times N}$, $\Lnu\in\mathds{C}^{N\times p}$, $\Ltheta\in\mathds{C}^{N\times h}$ which results in the evolution equation given by,  
\begin{align}
	\frac{\partial \vecu}{\partial t} =&  \Lu\vecu + \Lnu\vecnu + \Ltheta\vectheta + \NewNonLin(\vecu,\vecnu,\vectheta)  \nonumber.
\end{align}
The diagonal matrix for the harmonic angular frequencies is represented by $\LambdaH = diagm(\boldsymbol{\lambda}^{H})$. Similarly, $\boldsymbol{\lambda}^{P} = [\lambda^{P}_{1};\lambda^{P}_{2};\ldots;\lambda^{P}_{p}]$ is defined as a vector of length $p$, with all $\lambda^{P}_{i} = 0$ identically, for $i\in 1,\ldots, p$. These may be thought of parameter eigenvalues or parameter angular frequencies depending on the point of view. $\LambdaP = diagm(\boldsymbol{\lambda}^{P})$ is defined as an $p\times p$ null matrix, to represent the trivial linear evolution matrix for the parameter variables $\vecnu$. The extended system including the evolution equations for the newly introduced variables $\vecnu$ and $\vectheta$, henceforth referred to as the \emph{extended variables}, is given by,
\begin{subequations}
	\begin{align}
...
An overhead hat notation, $\widehat{\cdot}$ , is used to refer to variables and operators associated with the extend space, so that the new state vector, nonlinear term and the linearized direct and adjoint operators are denoted by $\efield{u}$, $\ENonLin$, $\ExtL$ and $\ExtL^{\dagger}$, that are defined as,
\begin{align}
...
\begin{align}
	...
	\end{bmatrix}.
\end{align}
The full system is now $M$-dimensional, with $M=N+p+h$, and may be represented compactly as,
\begin{equation}
...
The extended system has a larger center subspace of dimension $m = n+p+h$, as compared to the original system due to the $p+h$ additional variables introduced. While the extension of the system seems almost trivial, the eigenvector structure in the new dimensions however is far from trivial. Nevertheless, the extended eigenspace structure can be deduced directly from the properties of the original system. It is useful to examine the structure of the extended subspace since the adjoint extended critical subspace does have a special structure which has important practical implications for the asymptotic evaluation of the center manifold of the extended system.

There are however some subtleties involved when the system extension introduces new eigenvalues that are resonant with the original linear system $\Lu$. In such a scenario the system extension leads to a generalized eigenspace for the resonant eigenvalues. This is demonstrated in the next subsections where the subspace structure is evaluated in detail for both the resonant and non-resonant cases. This is first done for the extended operator $\ExtL$ and then for the adjoint operator $\ExtL^{\dagger}$. 

\subsection{Subspace structure for $\ExtL$}

Observe that if $\Tuple{\lambda^{C}_{i}, \Devec_{i}}$ is a critical eigenpair of the original linearized operator $\Lu$, then its extension to $\ExtL$ satisfies,
\begin{subequations}
	\begin{align}
...
\begin{subequations}
	\begin{align}
		\label{eqn:zeta_i_l}
		\vfield{e}^{P}_{i} = [\overbrace{0;0;\ldots;0}^{i-1};1;\overbrace{0;0;\ldots;0}^{p-i}], \\
		%
		\vfield{e}^{H}_{i} = [\underbrace{0;0;\ldots;0}_{i-1};1;\underbrace{0;0;\ldots;0}_{h-i}].
	\end{align}
\end{subequations}

If the original linear system $\Lu$ is non-singular then, the structure of the $p$ parameter eigenpairs is given by $\Tuple{\lambda^{P}_{i}, \EDevec^{P}_{i}}$, which satisfy
\begin{subequations}
...
Similarly, when the harmonic forcing is non-resonant, the $h$ forcing modes, denoted as $\Tuple{\lambda^{H}_{i}, \EDevec^{H}_{i}}$, generated by system extension due to $\vectheta$ have the following structure,
\begin{subequations}
...
\begin{subequations}
	\begin{align}
		\GammaP = 	[\vfield{\gamma}^{P}_{1},\vfield{\gamma}^{P}_{2},\ldots,\vfield{\gamma}^{P}_{p}], \\
...
\begin{subequations}
	...
	\end{align}
\end{subequations}
with vectors $\vfield{\zeta}^{P}_{i}$ and $\vfield{\zeta}^{H}_{i}$ defined such that the $j^{th}$ component of $\vfield{\zeta}^{P}_{i}$ is given by,
\begin{equation}
	...
	\end{split}\right.
\end{equation}
$\forall j\in 1,\ldots,p$, and, the $j^{th}$ component of $\vfield{\zeta}^{H}_{i}$ is given by,
\begin{equation}
...
\begin{align}
	...
	\EAevec^{P}_{i} &= \begin{bmatrix}
		...
		\vfield{e}^{P}_{i}; &
		\veczero
	\end{bmatrix}, && \forall i \in 1,\ldots p,
\end{align}
...
\begin{align}
	...
	\end{bmatrix},
\end{align}
where, $\AevMatrix$ is the matrix of the adjoint critical eigenvectors of $\Lu^{\dagger}$. The matrices $\mathbf{Z}_{P}\in\mathds{C}^{p\times n}$ and $\mathbf{Z}_{H}\in\mathds{C}^{h\times n}$ are defined as,
\begin{subequations}
	\label{eqn:adjoint_vectors_zeta}
\begin{align}
	\mathbf{Z}_{P} =& [\vfield{\zeta}^{P}_{1}, \vfield{\zeta}^{P}_{2},\ldots,\vfield{\zeta}^{P}_{n}], \\
	%
	\mathbf{Z}_{H} =& [\vfield{\zeta}^{H}_{1}, \vfield{\zeta}^{H}_{2},\ldots,\vfield{\zeta}^{H}_{n}].
\end{align}
\end{subequations}
The matrix of vectors of the extended (adjoint) center subspace is denoted by, $\mathbf{\widehat{W}} = [\mathbf{\widehat{W}}_{C}, \mathbf{\widehat{W}}_{P}, \mathbf{\widehat{W}}_{H}]$ with the individual vectors denoted as $\EAevec_{i},\ i\in 1,2,\ldots,m$. It can be verified that,
\begin{align}
...
\begin{eqnarray}
	\label{eqn:khat}
	\Khat = \mathbf{\widehat{W}}^{\dagger}\ExtL\EDevMatrix = 
	\begin{bmatrix}
...
This completes the extension of the system and, the subspace structure for both the direct and adjoint systems can be readily identified. The extension of the tangent spaces is summarized in Algorithm~\ref{algo:extended_direct_tangent} and ~\ref{algo:extended_adjoint_tangent}.

It needs to be mentioned at this point that while the parameter perturbations and harmonic forcings have been treated as distinct classes, from the point of view of extended subspaces there is no real difference. One may simply define a single class of ``extended matrices'' as,
\begin{equation}
	\label{eqn:single_class}
\begin{aligned}
	\GammaE = [\GammaP, \GammaH], && \mathbf{Z}_{E} = [\mathbf{Z}_{P}; \mathbf{Z}_{H}], \\
	%
	\DevMatrix_{E} = [\DevMatrix_{P}, \DevMatrix_{H}], && \EDevMatrix_{C} = [\DevMatrix; \veczero], \\
	%
	\EDevMatrix_{E} = [\DevMatrix_{E}; \mathbf{I}_{p+h}], && \EDevMatrix = [\EDevMatrix_{C}, \EDevMatrix_{E}], \\
	%
	\EAevMatrix_{C} = [\AevMatrix; \mathbf{Z}_{E}], && \EAevMatrix_{E} = [\veczero; \mathbf{I}_{p+h}],  \\
	\EAevMatrix = [\EAevMatrix_{C}, \EAevMatrix_{E}], &&
\end{aligned} 
\end{equation}
and,
\begin{eqnarray}
	...
	\end{bmatrix},
\end{eqnarray}
and refer to parameter and forcing modes as simply extended modes. Algorithmically they are treated in an identical manner.
 ...
 \begin{algorithm}
	...
	\begin{algorithmic}[0]
		...
		\Ensure $\NonLin(\veczero,\vecMu) = \veczero;$ $(\partial\NonLin/\partial \vecu)|_{\vecu=\veczero} = \veczero$. 
		\Require Tangent Space: $\AevMatrix^{\dagger}\Lu \DevMatrix = \LambdaC$.
		\LComment{Center subspace dimension: $n$.} 
		\State \textbf{Construct:} Linear operators $\Lnu\in\mathds{C}^{N\times p}$ and $\Ltheta\in\mathds{C}^{N\times h}$ for extended variables $\vecnu,\vectheta$. 
		\Require Diagonal submatrices: $\LambdaP,\LambdaH$.
		\LComment{Build: $\GammaP \in \mathds{C}^{n\times p};\ \GammaH \in \mathds{C}^{n\times h}$.}
		\For {$(i \in 1,\ldots,p)$}
			\For{$(j\in 1,\ldots,n)$}
		...
		\State $\Lu\mathbf{V}_{P} + \Lnu\mathbf{I}_{p} = \mathbf{V}_{P}\LambdaP + \DevMatrix\GammaP$, \Comment{Eq.~\eqref{eqn:singular_pmode}}
...
\begin{algorithm}
	...
	\begin{algorithmic}[0]
		...
		\State \LComment{Build: $\mathbf{Z}_{P} \in \mathds{C}^{p\times n};\ \mathbf{Z}_{H} \in \mathds{C}^{h\times n}$.}
		\For{$(i\in 1,\ldots,n)$}
			\For {$(j \in 1,\ldots,p)$}
