...
The evolution law may arise from a finite set of ordinary differential equations or, may be a finite representation of an infinite dimensional partial differential equation as often arises from the numerical discretization of the space-time evolution equations of physical systems. Typically, physical systems will have real parameters $\vecmu$ however, allowing for complex parameters presents no additional difficulty and we assume that $\vecmu \in \mathds{C}^{l}$. Without loss of generality one may assume $\vecu = \veczero$ to be a fixed point of the system for a certain set of parameters $\vecmu = \vecMu$, \ie,
\begin{align}
	\label{eqn:fixed_point}
	\vfield{f}(\veczero,\vecMu) = \veczero.
\end{align}
At the bifurcation point one may then divide the evolution operator into its linear and non-linear parts as,
\begin{align}
	\frac{\partial \vecu}{\partial t} = \Lu \vecu + \NonLin(\vecu,\vecMu)
\end{align}
where,
\begin{align}
	\Lu =& \left[\left.\dfrac{\partial \vecf}{\partial \vecu}\right|_{(\vecu=\veczero,\vecmu=\vecMu)}\right] \nonumber \\
	\mathcal{N}(\vecu,\vecMu) =& \vfield{f}(\vecu,\vecMu) - \Lu\vecu. \nonumber
\end{align}
...
Additionally, $\LambdaC$ is defined as an $n\times n$ diagonal matrix consisting of the critical eigenvalues of $\Lu$, with the implicit assumption of diagonalizability of $\Lu$ within the critical subspace. The ordering of the eigenvalues in $\LambdaC$ and eigenvectors in $\DevMatrix$ and $\AevMatrix$ is consistent with each other so that we have the eigenvalue relations,
\begin{align}
...
\begin{align}
	...
	\vecf^{H}(\vectheta) =\sum_{i=1}^{h}\vecf^{H}_{i}\theta_{i}(t); && \theta_{i} = e^{\lambda^{H}_{i}t},
\end{align}
...
The (time-independent) shape of the $i^{th}$ harmonic forcing is denoted as $\vecf_{i}$.
Denoting the new harmonically forced evolution operator as,
\begin{align}
	\vecf'(\vecu,\vecnu,\vectheta) =  \vecf(\vecu,\vecMu + \vecnu) + \vecf^{H}(\vectheta),
\end{align}
one may again separate out the linear and non-linear parts as,
\begin{align}
	%\label{eqn:split_operators}
	\Lu = \left[\left.\dfrac{\partial \vfield{f}'}{\partial \vecu}\right|_{(\vecu=\veczero,\vecnu=\veczero,\vectheta=\veczero)}\right] = \left[\left.\dfrac{\partial \vfield{f}}{\partial \vecu}\right|_{(\vecu=\veczero,\vecmu=\vecMu)}\right], \nonumber \\
	%
	\Lnu = \left[\left.\frac{\partial \vfield{f}'}{\partial \vecnu}\right|_{(\vecu=\veczero,\vecnu=\veczero,\vectheta=\veczero)}\right] =
	 \left[\left.\frac{\partial \vfield{f}}{\partial \vecmu}\right|_{(\vecu=\veczero,\vecmu=\vecMu)}\right], \nonumber \\
	 %
	 \Ltheta = \left[\left.\frac{\partial \vfield{f}'}{\partial \vectheta}\right|_{(\vecu=\veczero,\vecnu=\veczero,\vectheta=\veczero)}\right] =
	 \left[\left.\frac{\partial \vfield{f}^{H}}{\partial \vectheta}\right|_{(\vectheta=\veczero)}\right] \nonumber, \\
	 %
...
\begin{subequations}
	\begin{align}
			...
			\frac{\partial \vecu}{\partial t} =& \Lu\vecu + \Lmu\vecnu +\Ltheta\vectheta + \NonLin'(\vecu,\vecnu,\vectheta) \\
			%
...
\begin{subequations}
	...
	\begin{align}
		\EDevec^{P}_{i} = [
	  	...
	  	(\lambda^{P}_{i}\mathbf{I} - \Lu)\Devec^{P}_{i} = \Lmu\vfield{e}^{P}_{i}, &&  (\lambda^{P}_{i} = 0). 
	\end{align}
\end{subequations}
...
\begin{equation}
	...
	\left\lbrace
	\begin{split}
		\Aevec^{\dagger}_{j}\Lmu\vfield{e}^{P}_{i},  && \textit{if } \lambda^{P}_{i} = \lambda^{C}_{j}, \\
		0,						 && \textit{otherwise.}	
	\end{split}\right.
	%
\end{equation}
\begin{subequations}
	...
	\begin{align}
		...
		\veczero ], && \forall i \in 1,\ldots l, \\
		%
		(\lambda^{P}_{i}\mathbf{I} - \Lu)\Devec^{P}_{i} = \Lmu\vfield{e}^{P}_{i} - \DevMatrix\vfield{\gamma}^{P}_{i}, &&  (\lambda^{P}_{i} = 0), \\
		%
...
\begin{equation}
	...
	\left\lbrace
	\begin{split}
		(\Lmu^{\dagger}\Aevec_{i})_{j}/(\lambda^{C*}_{i} - \lambda^{P*}_{j}); && \textit{if}\ \lambda^{C*}_{i} \ne \lambda^{P*}_{j}, \\
...
\begin{equation}
	...
	\end{split}\right.
\end{equation}
$\forall j\in 1,\ldots,h$. Here, $(\Lmu^{\dagger}\Aevec_{i})_{j}$ refers to the $j^{th}$ component of the vector $(\Lmu^{\dagger}\Aevec_{i})$, and likewise for $(\Ltheta^{\dagger}\Aevec_{i})_{j}$. It may be verified that the definition is consistent with the eigenvalue equation, $(\lambda^{C*}_{i}\Ihat - \ExtL^{\dagger})\EAevec^{C}_{i} = 0$, for the non-resonant cases, and satisfies generalized eigenvalue equation, $(\lambda^{C*}_{i}\Ihat - \ExtL^{\dagger})^{2}\EAevec^{C}_{i} = 0$, for the resonant cases. 
