...
\end{equation}
As before, the higher index starts from the previous index. For the asymptotic expansions we will typically have $a=1$ and $b=m$, where $m$ is the dimension of the manifold. In this scenario the number of tuples in a multi-index tuple set $\IndexSet{k}{1,m}$ is analytically known to be $T_{k} = C(m+k-1,k)$, where, 
\begin{align}
	\label{eqn:binomial}
	C(n,k) = \binom{n}{k} = \dfrac{n!}{k! (n-k)!},
\end{align}
defines the binomial coefficient of $n$ and $k$. For example for a critical subspace dimension of $m=4$ one obtains $T_{2} = 10$ unknown fields $\vfield{y}_{a,b}$ at second order $(k=2)$ and, $T_{3} = 20$ unknown fields $\vfield{y}_{a,b,c}$ at third order $(k=3)$.
