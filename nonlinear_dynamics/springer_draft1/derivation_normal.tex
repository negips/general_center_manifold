...
\begin{subequations}
	...
	\begin{align}
		\ResolventHat{\omega_{a,b}}^{-1}\yhatbf_{a,b} + \sum^{m}_{i=1}\AsParamEl^{i}_{a,b}\vhatbf_{i} 
...
\end{align}
The resolvent operator is the key mathematical object defining the asymptotic expressions. $\ResolventHatOp(\omega_{a,b})$ is singular when $\omega_{a,b}$ is an eigenvalue of $\ExtL$, which is often referred to as a resonance condition. The normal form of the invariant manifold precisely resolves this singularity by requiring $\yhatbf_{a,b}$ to have a vanishing component along the singular direction. However, equation~\eqref{eqn:asymptotic_O2_ab} still needs to be balanced along the singular directions, which can only be done by $\sum^{m}_{j=1}\AsParamEl^{j}_{a,b}\vhatbf_{j}$. Multiplying equation~\eqref{eqn:asymptotic_O2_compact} with $\whatbf_{j}^{\dagger}$ and requiring that $\whatbf_{j}^{\dagger}\yhatbf_{a,b} = 0$ (in case of resonance), then provides us with the definition of $\AsParam_{a,b}$,
\begin{equation}
...
\begin{align}
	\begin{aligned}
		...
		\ResolventHat{\omega_{a,b,c}}^{-1}\yhatbf_{a,b,c}  + \sum^{m}_{j=1}\AsParamEl^{j}_{a,b,c}\vhatbf_{j} \\ 
...
\begin{subequations}
	...
	\end{align}
\end{subequations}
with the resonance condition arisising whenever, 
\begin{align}
...
The asymptotic terms can now be examined sequentially noting that by construction the extended variables vanish for the non-linear term, which has the form $\efield{f}_{NL} = [\vecf_{NL}; \veczero; \veczero]$, and hence so do all $\fhatbf_{i_{1},\ldots,i_{k}}$ derived from it. Starting at $\mathcal{O}(|\xbf|^{2})$ and $(i_{1},i_{2})=(1,1)$, one has the resolvent equation in the extended system,
\begin{align}
	\label{eqn:asymptotic_O2_11}
		\ResolventHat{\omega_{1,1}}^{-1}\yhatbf_{1,1} + \sum^{m}_{j=1}\AsParamEl^{j}_{1,1}\vhatbf_{j} 
		= \fhatbf_{1,1}
\end{align}
The extended variables vanish for $\fhatbf_{1,1}$ and therefore, regardless of resonance, they also vanish for $\yhatbf_{1,1}$, which then has the form $ \yhatbf_{1,1} = [\mathbf{y}_{1,1};\veczero;\veczero]$. Hence equation~\eqref{eqn:asymptotic_O2_11} reduces to,
\begin{align}
	\label{eqn:asymptotic_O2_11r}
	\Resolvent{\omega_{1,1}}^{-1}\mathbf{y}_{1,1} + \sum^{n}_{j=1}\AsParamEl^{j}_{1,1}\mathbf{v}_{j} 
	= \mathbf{f}_{1,1},
\end{align}
...
\begin{align}
	...
	\Resolvent{\omega_{1,2}}^{-1}\mathbf{y}_{1,2} + \sum^{n}_{j=1}\AsParamEl^{j}_{1,2}\mathbf{v}_{j} 
...
At $\mathcal{O}(|\xbf|^{3})$ the right hand side also has contributions from $\hhatbf^{L}_{i_{1},i_{2},i_{3}}$ that arise from the lower order asymptotic terms. However, these arise only due to the second order asymptotic terms $\yhatbf_{i_{1},i_{2}}$ and therefore, $\hhatbf^{L}_{i_{1},i_{2},i_{3}} = [\mathbf{h}^{L}_{i_{1},i_{2},i_{3}}; \veczero; \veczero]$. The same logical arguments presented earlier apply and hence all third order asymptotic terms also have the form $\yhatbf_{i_{1},i_{2},i_{3}} = [\mathbf{y}_{i_{1},i_{2},i_{3}}; \veczero; \veczero]$ and only require the standard linear system $\Lu$ for their evaluation. As noted earlier, equations for the higher order asymptotic terms have the same essential structure as the third-order equations and hence at all orders the asymptotic terms have the form $\yhatbf_{i_{1},\ldots,i_{k}} = [\mathbf{y}_{i_{1},\ldots,i_{k}}; \veczero; \veczero]$ and only require the standard linear system $\Lu$ for their evaluation. 
...
The entire procedure for the evaluation of the normal form is presented succinctly in algorithm~\ref{algo:normal_form}. 
 \begin{algorithm}
	...
	\begin{algorithmic}[0]
		\Require System: $\partial \vecu/\partial t = \Lu \vecu + \vecf(\vecu,\vecMu)$.
		\Ensure $\vecf(\vecu,\vecMu) = \veczero;$ $\partial\vecf/\partial \vecu = \veczero$. 
		\Require Tangent Space: $\mathbf{W}^{\dagger}\Lu \mathbf{V} = \LambdaC$.
		\LComment{Center subspace dimension: $n$.} 
		\Require Extension via $\vecnu, \vectheta$ and $\vecf_{NL}(\vecu,\vecnu)$.
		\State Extend Tangent space: $\What^{\dagger}\ExtL \Vhat = \Khat$
		...
		\State $\AsParamEl^{j}_{i_{1},\ldots,i_{k}} = \mathbf{w}^{\dagger}_{j}\mathbf{r}$
		\Else
		...
		\State $\mathbf{r}_{s} = \sum_{j=1}^{n}\mathbf{v}_{j}\AsParamEl^{j}_{i_{1},\ldots,i_{k}}$ 
		...
		\EndFor
		\EndFor \\
		\Return $\Khat, \Vhat,\What$,\\
		\Return ${y}_{i_{1},\ldots,i_{k}}, \AsParam_{i_{1},\ldots,i_{k}}, \forall k\in 2,\ldots,k_{max}$. 
	\end{algorithmic}
\end{algorithm}
