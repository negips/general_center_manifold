	...
	\begin{split}
		...
		& \Big[
		 ...
		 \sum^{m}_{i=b}\widehat{K}_{i,a}\yhatbf_{b,i}\delta^{a}_{b}\Big] 
		   \\
		& + \sum^{m}_{i=1}\AsParam^{i}_{a,b}\vhatbf_{i} - \ExtL\yhatbf_{a,b} = \fhatbf_{a,b}.
	\end{split}
\end{equation}
...
To keep the notation compact, the non-vanishing off-diagonal coupling terms will be represented as $\hhatbf_{a,b}$, defined for second order as,
\begin{equation}
...
\begin{equation}
	...
	\left[(\widehat{K}_{a,a} + \widehat{K}_{b,b})\mathbf{I} - \ExtL\right]\yhatbf_{a,b} \\
	+ \sum^{m}_{i=1}\AsParam^{i}_{a,b}\vhatbf_{i} = \fhatbf_{a,b} - \hhatbf^{O}_{a,b} 
	\end{split}
\end{equation}
This equation is singular when $(\widehat{K}_{a,a} + \widehat{K}_{b,b})$ is an eigenvalue of $\ExtL$, which is often referred to as a resonance condition. The normal form of the invariant manifold precisely resolves this singularity by requiring $\yhatbf_{a,b}$ to have a vanishing component along the singular direction. However, equation~\eqref{eqn:asymptotic_O2_ab} still needs to be balanced along the singular directions, which can only be done by $\sum^{m}_{i=1}\AsParam^{i}_{a,b}\vhatbf_{i}$. Multiplying equation~\eqref{eqn:asymptotic_O2_compact} with $\whatbf_{i}^{\dagger}$ and requiring that $\whatbf_{i}^{\dagger}\yhatbf_{a,b} = 0$ (in case of resonance), then provides us with the definition of $\AsParam^{j}_{a,b}$,
\begin{equation}
	\label{eqn:g2_definition}
	\AsParam^{i}_{a,b} =
	\begin{cases}
				\whatbf_{i}^{\dagger}(\fhatbf_{a,b} - \hhatbf^{O}_{a,b}); & \textit{if}\  (\widehat{K}_{a,a}+\widehat{K}_{b,b}) = \widehat{K}_{i,i},\\
				0,														& \textit{otherwise},
	\end{cases}
\end{equation}
where, $\whatbf_{i}$ is the $i^{th}$ adjoint eigenvector of $\ExtL$ as defined earlier. Since the system is lower triangular the terms $\hhatbf^{O}_{a,b}$ are always known apriori when solving the system sequentially. 
...
\begin{align}
	...
	\begin{split}
		...
		%
		%
		& + \sum^{m}_{i=1}\sum^{[1,m]}_{(3)} \AsParam^{i}_{j_{1},j_{2},j_{3}}\vhatbf_{i}x_{j_{1}}x_{j_{2}}x_{j_{3}}  \\ 
		%
...
\begin{equation}
	...
	\left[(\widehat{K}_{a,a} + \widehat{K}_{b,b} + \widehat{K}_{c,c})\mathbf{I} - \ExtL\right]\yhatbf_{a,b,c}  \\ 
	+ \sum^{m}_{i=1}\AsParam^{i}_{a,b,c}\vhatbf_{i} = \fhatbf_{a,b,c} - \hhatbf^{O}_{a,b,c} - \hhatbf^{L}_{a,b,c}
\end{split}
\end{equation}
The resolution of the system singularities due to the resonance condition, $(\widehat{K}_{a,a} + \widehat{K}_{b,b} + \widehat{K}_{c,c}) = \widehat{K}_{i,i}$, provides us with the third order terms of the reduced representation,
\begin{equation}
	\label{eqn:g3_definition}
	\AsParam^{i}_{a,b,c} =
	\begin{cases}
		\begin{split}
			\whatbf_{i}^{\dagger}(\fhatbf_{a,b,c} - \hhatbf^{L}_{a,b,c}\\
			 - \hhatbf^{O}_{a,b});
		\end{split}
		 & \begin{split}
			\textit{if},\ (\widehat{K}_{a,a}+\widehat{K}_{b,b}\\
			 + \widehat{K}_{c,c}) = \widehat{K}_{i,i}
		\end{split} \\
		0,														& \textit{otherwise},
	\end{cases}
\end{equation}
and $\whatbf^{\dagger}_{i}\yhatbf_{a,b,c} = 0$ if the resonance condition holds. 
