...
To simplify notation for multiple indices used hereafter, a multi-dimensional index set $\IndexSet{k}{a,b}$ is introduced, which is an ordered set of all  $k$-indices $(i_{1},i_{2}\ldots,i_{k})$, defined as 
\begin{equation}
	\label{eqn:index_set}
	\begin{split}
	...
	\end{split}
\end{equation}
where, $i_{1},i_{2},\ldots,i_{k}$, are the index variables and $[a,b]$ specify the lower and upper bounds of the indices. Obviously the indices $i_{1}$ \etc\ are dummy indices however their ordering is important since the higher index always starts from the previous index. Hence the index-set $\IndexSet{2}{1,3}$ is the set containing the tuples, $(i_{1},i_{2}) \in \{(1,1), (1,2), (1,3), (2,2), (2,3), (3,3)\}$. Similarly, multiple nested summations are denoted as,
\begin{equation}
	\label{eqn:summation}
	\sum_{(k)}^{[a,b]} = \sum_{i_{1}=a}^{b}\ \sum_{i_{2}=i_{1}}^{b}\ldots\sum_{i_{k}=i_{k-1}}^{b}
\end{equation}
As before, the higher index starts from the previous index. For the asymptotic expansions we will typically have $a=1$ and $b=m$, where $m$ is the dimension of the manifold. For such cases the number of $k$-tuples in a multi-index tuple set $\IndexSet{k}{1,m}$ is analytically known to be $T^{k}_{m} = C(m+k-1,k)$, where, 
\begin{align}
	\label{eqn:binomial}
...
One may find the exposition on the implementation of the parameterization method in previous works of \citet{haro16} and \citet{jain22}. To be clear, the application of the parameterization method to an extended system like equation~\eqref{eqn:evolution_extended} follows the same steps as outlined in those earlier works. However, in order to show that the resolvent calculations in fact only require the original linear system, and not the extended one, an explanation of the key steps of the method is necessary.

Consider the extended system as defined by equation~\eqref{eqn:evolution_extended}, represented in a compact form by equation~\eqref{eqn:evolution_extended_compact}, which has an $m = n + l + h$ dimensional critical subspace. Assume $\xbf = [x_{1}; x_{2};\ldots;x_{m}]$ is an $m$-dimensional vector of coordinates that parameterize the center-manifold of the extended system. The evolution of the system along the center-manifold in the reduced coordinates is assumed to be given by $\dt{\xbf} = \Param(\xbf)$, where, $\Param$ is a vector valued function. The system solution on the center-manifold corresponding to a point $\xbf$ on the reduced system is given by $\mathcal{Y}(\xbf)$. At this stage neither $\Param(\xbf)$ nor $\mathcal{Y}(\xbf)$ have been specified in any form, since, the parameterization can be arbitrarily chosen. In order to obtain a normal form of the reduced system one may expand both $\Param(\xbf)$ and $\mathcal{Y}(\xbf)$ as a formal power series in $\xbf$ and constrain the asymptotic form of $\Param$ to be in a normal form for the parameterized coordinates $\xbf$. The asymptotic form of $\mathcal{Y}$ is obtained by introducing the asymptotic expansions into equation~\eqref{eqn:evolution_extended_compact} and solving the terms order-by-order. 
...
\begin{align}
	\label{eqn:asymptotic_reduced_normal_form}
	\begin{split}
...
\begin{align}
	...
	\end{split}
\end{align}
which is obtained via the grouping of terms with similar powers of $\xbf$. 
...
\begin{align}
	...
	\end{split}
\end{align}
Here $\AsParam^{\alpha}_{i_{1}}$ represents the $\alpha^{th}$ element of the vector $\AsParam_{i_{1}}$. 
...
\begin{subequations}
	...
	\end{align}
\end{subequations}
which reduces equation~\eqref{eqn:asymptotic_O1} to equation~\eqref{eqn:extended_generalized_eigenvalue}. This then satisfies the tangency condition of the invarian manifold, \ie, at the origin the center-manifold is tangent to the center subspace. Subsequently, $\widehat{K}_{i,j}$ will be used for all the first order terms of the type $\AsParam^{i}_{j}$ and the first order vectors $\yhatbf_{i}$ will be replaced by the respective eigenvectors $\vhatbf_{i}$. The reduced matrix $\mathbf{\widehat{K}}$ is responsible for the cross-coupling of different unknowns at each order of the asymptotic expansion. 
...
\begin{align}
	...
	\end{split} 
\end{align}
Collecting terms for a particular asymptotic term $x_{a}x_{b}$, where $(a,b)\in\IndexSet{2}{1,m}$, equation~\eqref{eqn:asymptotic_O2} reduces to,
\begin{equation}
	\label{eqn:asymptotic_O2_ab}
	\begin{split}
		&[\widehat{K}_{a,a} + \widehat{K}_{b,b}]\yhatbf_{a,b} + 
		\Big[ \sum^{a}_{\substack{i=1 \\ i\neq b}}\widehat{K}_{i,b}\yhatbf_{i,a} +
		\sum^{b}_{\substack{i=1 \\ i\neq a}}\widehat{K}_{i,a}\yhatbf_{i,b}\Big] + \\
		& \Big[
		 \sum^{m}_{\substack{i=a \\ i\neq b}}\widehat{K}_{i,b}\yhatbf_{a,i} +
		 \sum^{m}_{\substack{i=b \\ i\neq a}}\widehat{K}_{i,a}\yhatbf_{b,i}  \Big] 
		 + \sum^{m}_{j=1}\AsParam^{j}_{a,b}\vhatbf_{j}  \\
		& - \ExtL\yhatbf_{a,b} = \fhatbf_{a,b}.
	\end{split}
\end{equation}
