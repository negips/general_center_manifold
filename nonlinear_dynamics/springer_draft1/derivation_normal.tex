...
Due to the diagonal terms of $\Khat$ one obtains an expression of the type $[(\widehat{K}_{a,a} + \widehat{K}_{b,b})\mathbf{I} - \ExtL]\yhatbf_{a,b}$, which is singular when $(\widehat{K}_{a,a} + \widehat{K}_{b,b})$ is an eigenvalue of $\ExtL$. The normal form of the invariant manifold precisely resolves this singularity by requiring $\yhatbf_{a,b}$ to have a vanishing component along the singular direction. However, equation~\eqref{eqn:asymptotic_O2_ab} still needs to be balanced along the singular directions, which can only be done by $\sum^{m}_{j=1}\AsParam^{j}_{a,b}\vhatbf_{j}$. This defines the terms of the reduced representation $\AsParam^{j}_{a,b}$ such that,
\begin{equation}
	\AsParam^{j}_{a,b} =
	\begin{cases}
				\whatbf_{j}^{\dagger}(\fhatbf_{a,b} - \hhatbf_{a,b}); & \textit{if}\  \widehat{\lambda}_{j} = (\widehat{K}_{a,a}+\widehat{K}_{b,b}) \\
				0														& \textit{otherwise}.
	\end{cases}
\end{equation}
