...
Furthermore, we assume that $\vecMu$ represents the parameters of the system at a bifurcation point. It is generically assumed that the system undergoes a multiple co-dimension bufurcation at $\vecMu$ and that the linearized operator $\Lu$ has a dicrete set of $N$ critical eigenvalues $\lambda^{c}$, with zero growth rate $(\mathfrak{Re}(\lambda^{c}) = 0)$. Of those, there are 
$n$ purely real critical eigenvalues with $\mathfrak{Re}(\lambda^{c}) = 0, \mathfrak{Im}(\lambda^{c}) = 0$, and $m = N - n$ purely imaginary critical eigenvalues with $\mathfrak{Re}(\lambda^{c}) = 0, \mathfrak{Im}(\lambda^{c}) \ne 0$. For real systems $m$ is an even number since the complex eigenvalues occur in conjugate pairs. Accordingly, the system has $N$ critical eigenpairs $(\lambda^{c}_{i},\vecv_{i}), i\in 1\ldots N$, where $\vecv_{i}$ represents the eigenvector corresponding to $\lambda_{i}$. 

The adjoint of the direct linearized operator $\Lu$ is denoted as $\Lu^{\dagger}$ which also has $N$ critical eigenpairs $(\lambda^{c*}_{i}, \vecw^{\dagger}_{i})$. The scaling of the eigenvectors of the direct and adjoint operators may be defined such that they satisfy the birthogonality condition
\begin{eqnarray}
...
\begin{align}
	...
	\vecf_{H}(t) =\sum_{i=1}^{h}\vecf_{i}\theta_{i}(t); && \theta_{i} = \epsilon_{i}e^{\omega_{i}t},
\end{align}
where, the various $\theta_{i}$ are treated as the dynamic variable, $\omega_{i}$ is purely imaginary, with the imaginary part representing the angular frequency of the forcing. $\vecf_{i}$ represents the shape of the $i^{th}$ harmonic forcing and $\epsilon_{i}$ is a scale factor. Constant forcing terms with zero frequency are readily incorporated when a particular (or multiple) $\omega_{i}=0$. The vector of the various $\theta_{i}$ is denoted as $\vectheta = [\theta_{1};\theta_{2};\ldots;\theta_{h}]$, and the vector of angular frequencies denoted $\boldsymbol{\omega}_{H} = [\omega_{1};\omega_{2};\ldots;\omega_{h}]$. The resulting evolution equation is 
\begin{align}
	\frac{\partial \vecu}{\partial t} =& \vecf(\vecUb + \vecu,\vecMu + \vecnu) + \sum_{i=1}^{h}\vecf_{i}\theta_{i}; \nonumber.
\end{align}
The diagonal matrix for the harmonic angular frequencies is represented by $\mathbf{\Lambda}_{H} = diag(\boldsymbol{\omega}_{H})$. Similarly, $\boldsymbol{\omega}_{P} = [0;0;\ldots;0]$ is defined as a null vector of length $l$ and $\mathbf{\Lambda}_{P}$ is defined as an $l\times l$ null matrix, to represent the trivial eigenvalues and linear evolution matrix of the parameter variables $\vecnu$. The extended system including the evolution equations for the newly introduced variables $\vecnu$ and $\vectheta$, henceforth referred to as the \emph{extended variables}, is given by,
\begin{subequations}
	\begin{align}
...
\end{align}
The superscript $^{\dagger}$ refers to the operator adjoint and in the case of matrices it refers to the complex-conjugate transpose of the matrix. The full system may then be represented compactly as
\begin{equation}
...
The $l$ eigenpairs generated by the system extension due to $\vecnu$ have the structure of the type $(\lambda_{i}, [\vecv^{p}_{i};\vfield{e}^{l}_{i}; \vfield{0}])$, which satisfy
\begin{subequations}
	\label{eqn:nonsingular_pmode}
	\begin{align}
		(\lambda_{i}\mathbf{I} - \Lu)\vecv^{p}_{i} = \Lmu(\vfield{e}^{l}_{i});&&  \forall i\in 1,\ldots l \\
		\lambda_{i} = (\boldsymbol{\omega}_{P})_{i} = 0 \nonumber. && 
	\end{align}
\end{subequations}
...
Similarly, the $h$ eigenpairs generated by system extension due to $\vectheta$ have the structure of the type $(\lambda_{i}, [\vecv^{h}_{i};\vfield{0}; \vfield{e}^{h}_{i} ])$, which satisfy
\begin{subequations}
	\label{eqn:nonsingular_hmode}
	\begin{align}
		(\lambda_{i}\mathbf{I} - \Lu)\vecv^{h}_{i} = \Ltheta(\vfield{e}^{h}_{i});&&  \forall i\in 1,\ldots h \nonumber \\
		\lambda_{i} = (\boldsymbol{\omega}_{H})_{i} \nonumber. && 
	\end{align}
\end{subequations}
These will be referred to has \emph{forcing modes}. 

In the adjoint case, for any eigenpair $(\lambda^{*},\vecw^{\dagger})$ of the original adjoint operator $\Lu^{\dagger}$, the corresponding eigenpair of the extended operator $\ExtL^{\dagger}$ is given by $(\lambda^{*}, [\vfield{w}^{\dagger}; \vfield{\zeta}^{\dagger}_{P}; \vfield{\zeta}^{\dagger}_{H}])$, with,
\begin{subequations}
	\begin{eqnarray}
		\label{eqn:extended_adjoint_eigenvector}
		(\lambda^{*}\mathbf{I} - 	\mathbf{\Lambda}^{\dagger}_{P})\vfield{\zeta}^{\dagger}_{P} = \Lmu^{\dagger}\cdot\vecw^{\dagger}, \\
		%
		(\lambda^{*}\mathbf{I} - 	\mathbf{\Lambda}^{\dagger}_{H})\vfield{\zeta}^{\dagger}_{H} = \Ltheta^{\dagger}\cdot\vecw^{\dagger}
	\end{eqnarray}
\end{subequations}
...
Again, eigenpairs $(\lambda, \vfield{v})$ of the original system $\Lu$ can be trivially extended to $(\lambda, [\vecv; \vfield{0}; \vfield{0}])$ which satisfy the eigenvalue equation for $\ExtL$. 
...
\begin{equation}
	(\vfield{\gamma}^{i}_{P})_{j} =
	\left\lbrace
	\begin{split}
		(\vecw^{\dagger}_{j})^{H}\Lmu(\vfield{e}^{l}_{i}),  && \textit{if } (\boldsymbol{\omega}_{P})_{i} = \lambda^{c}_{j} \\
		0,						 && \textit{otherwise.}	
	\end{split}\right.
	%
\end{equation}
Then $(\lambda_{i}, [\vecv^{p}_{i};\vfield{e}^{l}_{i}; \vfield{0}])$ forms a generalized eigenmode which satisfies the modified relation,
\begin{subequations}
	\label{eqn:singular_pmode}
	\begin{align}
		(\lambda_{i}\mathbf{I} - \Lu)\vecv^{p}_{i} =& \Lmu(\vfield{e}^{l}_{i}) - \mathbf{V}\vfield{\gamma}^{i}_{P};&  \forall i\in 1,\ldots l, \\
		%
		\lambda_{i} = (\boldsymbol{\omega}_{P})_{i} =& 0, & \\
		%
		(\vecw^{\dagger}_{j})^{H}\vecv^{p}_{i} =& 0; & \textit{if } (\boldsymbol{\omega}_{P})_{i} = \lambda^{c}_{j}. 
	\end{align}
\end{subequations}

Similarly, for the $i^{th}$ \emph{extended forcing mode}, define a vector $\vfield{\gamma}^{i}_{H} \in \mathds{C}^{N}$, where the $j^{th}$ element of the vector is given by,
\begin{equation}
	(\vfield{\gamma}^{i}_{H})_{j} =
	\left\lbrace
	\begin{split}
		(\vecw^{\dagger}_{j})^{H}\Ltheta(\vfield{e}^{l}_{i}),  && \textit{if } (\boldsymbol{\omega}_{H})_{i} = \lambda^{c}_{j} \\
		0,						 && \textit{otherwise.}	
	\end{split}\right.
	%
\end{equation}
Then $(\lambda_{i}, [\vecv^{q}_{i};\vfield{e}^{h}_{i}; \vfield{0}])$ forms a generalized eigenmode which satisfies the modified relation,
\begin{subequations}
	\label{eqn:singular_hmode}
	\begin{align}
		(\lambda_{i}\mathbf{I} - \Lu)\vecv^{q}_{i} =& \Ltheta(\vfield{e}^{h}_{i}) - \mathbf{V}\vfield{\gamma}^{i}_{H};&  \forall i\in 1,\ldots l, \\
		%
		\lambda_{i} = (\boldsymbol{\omega}_{P})_{i},& & \\
		%
		(\vecw^{\dagger}_{j})^{H}\vecv^{q}_{i} =& 0; & \textit{if } (\boldsymbol{\omega}_{H})_{i} = \lambda^{c}_{j}. 
	\end{align}
\end{subequations}
